...
TGSD_voronoi <- function(voronoi_input) {
  ...
  fullpoints[is.na(fullpoints)] <-
    0 # replaces NA values for zero line with zero
  ...
  utm <-
    sp::spTransform(ll, CRS(utm_convert)) # transform from lat long to UTM (projected)
  
  vdf <-
    as.data.frame(cbind(coordinates(utm), fullpoints$gm2)) # get utm coordinates and mass into data frame
  colnames(vdf) <- c("x", "y", "z")
  
  vordat_spdf <-
    ggvoronoi::voronoi_polygon(vdf, x = "x", y = "y") # voronoi tesselation
  vor_area <-
    rgeos::gArea(vordat_spdf, byid = TRUE) #calculate area of each voronoi cell
  vordat_df <-
    as.data.frame(cbind(vordat_spdf$x, vordat_spdf$y, vordat_spdf$z, vor_area)) #assign area to lat long pair
  colnames(vordat_df) <- c("x", "y", "z", "area")
  
  TGSDcalc <-
    subset(vordat_df, z > 0) #filter to exclude zero points
  
  calcpoints <-
    cbind(llpoints, TGSDcalc) #combine GSD dataframe with area dataframe
  calcpoints$totarea <- sum(calcpoints$area)
  calcpoints$prop <-
    calcpoints$area / calcpoints$totarea #weighting by proportion of voronoi cell area
  
  gsdpoints <-
    calcpoints[, grepl("phi", names(calcpoints))] #isolate the GSDs
  ...
  phi_bins <-
    data.table::tstrsplit(phi_bins, "phi")[[2]] # remove prefix
  phi_bins <-
    as.numeric(gsub("\\_", "-", phi_bins)) # convert to numeric and replace underscore with minus sign
...
coords2country <- function(latlongpoints) {
  ...
  pointsSP <-
    sp::SpatialPoints(llonly, proj4string = CRS(proj4string(countriesSP)))
...
TGSD_plots <- function(TGSD_input) {
  ...
  mm_plotting <-
    data.frame(breaks = -log2(c(1e-3, 10e-3, 100e-3, 1000e-3, 10000e-3)),
               labels = c(0.001, 0.01, 0.1, 1, 10))
  phi_plotting <-
    data.frame(breaks = seq(min(sf_out$TGSD$phi), max(sf_out$TGSD$phi),
                            by = 2))
  
  y_ax_fact <-
    100 / ceiling(max(sf_out$TGSD$PDF)) # factor for getting CDF and PDF on same plot
  plot_limits <-
    data.frame(
      xmin = ceiling(min(voronoi_input$llzeros$long)) - 2,
      xmax = ceiling(max(voronoi_input$llzeros$long)) +
        2,
      ymin = ceiling(min(voronoi_input$llzeros$lat)) -
        2,
      ymax = ceiling(max(voronoi_input$llzeros$lat)) +
        2
    )
